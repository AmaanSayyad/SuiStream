...
const SubscribeModal = ({
  ...
  const getUSDCxBalance = async () => {
    try {
      ...
      const _balance = await usdcx.balanceOf({
        account: address,
  ...
  const handleSubscribe = async () => {
    const createFlowOperation = await createFlow(
      flowRate.toString(),
      toUserAddress
    );
    toast("Subscription Flow Started ��");
    const subscribeOperation = await superstream.subscribe(
      toUser,
      Number(flowRate.toString())
    );
          ...
          >
            <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-gray-800 shadow-xl rounded-2xl">
              ...
              <div>
                ...
                {flowRate &&
                  Number(
                    ethers.utils.formatEther(flowRate?.toString())
                  ).toFixed(8)}{" "}
                / second
              </div>
              <div className="flex gap-4 mt-4">
                <button
                  ...
                  className="w-full py-2  rounded-lg justify-center bg-emerald-500 group hover:bg-emerald-400"
                >
                ...
                >
                  <XIcon className="h-6 w-6 mr-2 group-hover:scale-110" />{" "}
                  Cancel{" "}
                </button>
              </div>
            </div>
